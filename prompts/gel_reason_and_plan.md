# GEL Chain-of-Thought Reasoning and Edit Planning

You are analyzing a knowledge graph to determine why retrieval failed and what minimal edits would fix it.
...
**Step 2 - Gap Analysis**: What is MISSING? What should exist but doesn't? Be specific about what facts or connections are absent.
...
**Step 4 - Edit Plan**: For each root cause, propose the MINIMAL edit to fix it.

## Available edit operations
- `CREATE_FACT`: Create a new fact node. Params: `fact_text` (string), `belief` (float, max 0.85), `source_episode_ids` (list of episode node IDs this fact is derived from), `concepts` (list of concept labels to connect to).
...
- Every fact MUST be a **concrete, positive, specific claim** extracted directly from an episode. Examples:
  - GOOD: "Nate's nickname for Joanna is Jo"
  - GOOD: "Joanna visited Indiana in July 2021"
...
- NEVER create a fact containing: "not specified", "not mentioned", "not clear", "unknown", "may include", "not explicitly", "but it is not", "no information", "does not"
...
- If the information cannot be found in any episode, DO NOT create a fact. Return empty edit_ops instead.
...
- Only create facts that DIRECTLY help answer the failed query — not adjacent or tangentially related topics.
...
- If no concrete facts can be extracted, return `"edit_ops": []`.
