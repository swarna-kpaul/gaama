"""
Baseline evaluation for MemoryBench Progressive Search.
========================================================
...
from progressive_search_eval import (
    load_dataset, compute_summary,
    ProgressiveSearchBaselineEval,
)


def main() -> None:
    parser = argparse.ArgumentParser(description="MemoryBench Progressive Search baseline evaluation")
    ...
    print("=" * 70, flush=True)
    print("MemoryBench Progressive Search — Baseline (Full Chat History)", flush=True)
    print(f"  entries:  {len(dataset)}", flush=True)
    print("=" * 70, flush=True)

    evaluator = ProgressiveSearchBaselineEval(
        data_dir=DATA_DIR,
        llm_model=LLM_MODEL,
