"""
MemoryBench Progressive Search Evaluation Module
=================================================
Two evaluation classes:
  - ProgressiveSearchSDKEval      GAAMA memory SDK retrieval-augmented evaluation
  - ProgressiveSearchBaselineEval Full chat-history baseline (perfect memory upper bound)

Each entry has 4-16 sequential questions forming a progressive search chain.
Each question builds on prior answers to narrow down a target entity.
The final question is a comprehensive synthesis of all prior clues.

Shared utilities:
  - load_dataset(path)       Load progressive_search.jsonl
...
def _strip_citations(text: str) -> str:
    """Remove document citation IDs like [63918] from reference answers."""
...
def _score_answer(
    ...
    """LLM judge: how factually accurate is the hypothesis vs reference answer?"""
    if not hypothesis:
        return {"reward": 0.0, "justification": "No hypothesis provided."}

    gt_clean = _strip_citations(ground_truth)

    prompt = (
        "You are evaluating a search answer. Compare the model's answer against "
        "the reference answer for factual accuracy.\n\n"
        f"Reference answer:\n{gt_clean}\n\n"
        f"Model's answer:\n{hypothesis}\n\n"
        "Score based on factual overlap:\n"
        "- 1.0: The model identifies the same key entity/fact as the reference.\n"
        "- 0.5-0.9: Partially correct — some facts match but key details differ.\n"
        "- 0.1-0.4: Minimal overlap — mostly wrong but some related information.\n"
        "- 0.0: Completely wrong or unrelated.\n\n"
        "Focus on whether the core answer (entity name, key fact) matches, "
        "not on formatting or explanation style.\n\n"
...
class ProgressiveSearchSDKEval:
    """Evaluate Progressive Search entries using GAAMA memory SDK retrieval.

    Flow per entry (4-16 sequential questions):
      Q0: question → LLM → hypothesis
      Q1+: retrieve memory from question, memory + question → LLM → hypothesis
      After every question: ingest question + reference answer
    """
    ...
    def evaluate_entry(self, entry: dict) -> List[Dict[str, Any]]:
        """Evaluate one entry (4-16 sequential search questions) using SDK retrieval."""
        ...
        for step_idx in range(num_questions):
            question = questions[step_idx]
            answer = answers[step_idx]
            is_final = step_idx == num_questions - 1
            ...
            # --- Score ---
            scoring = _score_answer(answer, hypothesis, llm, self.eval_model)
            ...
            print(
                f"  {marker} entry_{entry_id} step{step_idx}{tag}: "
                f"reward={scoring['reward']:.2f}",
                flush=True,
            )

            # --- Ingest question + reference answer into memory ---
            gt_clean = _strip_citations(answer)
            event = TraceEvent(
...
class ProgressiveSearchBaselineEval:
    """Evaluate Progressive Search entries using full chat history.

    Flow per entry:
      Q0: question → LLM → hypothesis
      Q1+: chat history (all prior Q&A) + question → LLM → hypothesis
    """
...
def compute_summary(results: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Compute per-step, final-question, and overall reward summary."""
    if not results:
    ...
    for r in results:
        ...
        if r["is_final"]:
            ...
            non_final_rewards.append(r["reward"])

    n = len(all_rewards)
    correct = sum(1 for r in all_rewards if r > 0.5)

    print("\n" + "=" * 70, flush=True)
    print("REWARD SUMMARY — Progressive Search", flush=True)
    print("=" * 70, flush=True)
    print(f"\n  OVERALL: n={n}  reward={overall:.4f}  acc={correct/n:.4f}", flush=True)

    # Final vs non-final
    print(f"\n{'--- Final vs Non-Final ---':^70}", flush=True)
    if non_final_rewards:
        avg_nf = sum(non_final_rewards) / len(non_final_rewards)
        c_nf = sum(1 for r in non_final_rewards if r > 0.5)
        print(f"  Non-final:  n={len(non_final_rewards):4d}  reward={avg_nf:.4f}  acc={c_nf/len(non_final_rewards):.4f}", flush=True)
    if final_rewards:
        avg_f = sum(final_rewards) / len(final_rewards)
        c_f = sum(1 for r in final_rewards if r > 0.5)
        print(f"  Final:      n={len(final_rewards):4d}  reward={avg_f:.4f}  acc={c_f/len(final_rewards):.4f}", flush=True)

    # By step (only show steps with enough data)
    print(f"\n{'--- By Step ---':^70}", flush=True)
    by_step = {}
    for step in sorted(step_rewards):
        rw = step_rewards[step]
        if len(rw) < 5:
            continue  # skip steps with very few entries
        avg = sum(rw) / len(rw)
    ...
    return {
        ...
        "by_step": by_step,
        "final_reward": sum(final_rewards) / len(final_rewards) if final_rewards else 0.0,
    }
